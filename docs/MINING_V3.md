...
The [full candidate follow-up](MINING_V3_FULL_DEEP.md) completed all 4,345 distinct states selected by this screen under the same depth-20/24 contract on 15 September. It includes the initial 200, with their original evidence checked before reuse. Its [validated aggregate](../results/mining_v3_full_deep.json) reports 2,379 engine-stable positions and 1,398 also retaining the full candidate criterion; the first-pass totals above remain unchanged. Unlike the initial batch, the census includes BOT-tagged, previously public and unresolved sources and permits several states from one game. Those differences matter when interpreting its outcomes. None of the retained positions has been approved for the trainer.
