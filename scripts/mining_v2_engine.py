...
def search(engine, board, depth, seconds, roots=None, multipv=None):
    ...
    # analyse() merges InfoDicts with dict.update, retaining stale bound flags.
    # Merge streamed updates ourselves and reset bounds on each new score event.
    latest = {}
    with engine.analysis(board, chess.engine.Limit(depth=depth, time=seconds),
    ...
    infos = [latest[index] for index in sorted(latest) if 'score' in latest[index]]
    if not infos:
