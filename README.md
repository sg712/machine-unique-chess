...
The recorded move matched the exact engine choice in 1,199 selected positions; the other 3,956 are nonmatches, not necessarily errors. These historical source games include bot accounts, so the rates are not human success rates. In 2,034 positions, the saved runner-up is less than 20cp behind. The trainer has an additional curation filter: depth 18, retained top move, at least 70cp above the runner-up when built.
...
**Full colour expansion:** 123,405 genuine White observations have now been added, giving **124,405 White and 124,405 Black observations** including the pilot. All new histories replay correctly; repeated historical states are flagged. Recovering the original games corrected 207 rating joins and identified 17,931 historical observations from BOT-tagged games. The full Stockfish 18/Maia3 first-pass analysis is complete for all 248,810 observations, using the same settings for both colours. The selected candidates subsequently completed the deeper checks below; explanations and chess review are still required before adding puzzles. [Expansion methods](docs/MINING_V3.md), [dataset counts](results/mining_v3_dataset.json), [source audit](results/mining_v3_provenance.json) and [screening results](results/mining_v3_summary.json).
...
**Full candidate pool completed, 15 September:** all **4,345 distinct candidates** from 3,726 source games completed every legal-move check at depths 20 and 24. **2,379 met the engine-stability rule; 1,398 also retained the full model-based candidate criterion** (680 White, 718 Black). The 304,068 searches include 14,208 validated searches from the initial 200 positions and 289,860 new searches. The initial batch is part of this total. Source labels remain explicit, including BOT-tagged, public and unresolved games; these results do not measure human solving or learning. No positions have been added to the trainer from this census. [Full results and methods](docs/MINING_V3_FULL_DEEP.md), [frozen census](results/mining_v3_full_deep_selection.json) and [validated aggregate](results/mining_v3_full_deep.json).
